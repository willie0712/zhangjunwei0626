const translations = {
  zh: {
    brandText: "Willie Zhang的部落格",
    navAbout: "關於我",
    navSkills: "我的專長",
    navGallery: "攝影作品",
    navGaming: "遊戲時光",
    navGoals: "未來目標",
    navContact: "聯絡我",
    heroEyebrow: "攝影生活 / 遊戲編年史 / 2026 新篇章",
    heroTagline: "基隆二信高中生，用鏡頭收藏雨港日常，也把熱血寫進自己的故事。",
    heroQuote: "鏡頭記錄當下，熱血活在當下。",
    heroCta: "探索我的世界",
    heroIg: "追蹤 Instagram",
    heroStatusTitle: "目前狀態",
    heroStatus: "2026 專注攝影、剪輯與生活成長",
    aboutEyebrow: "About Willie",
    aboutTitle: "關於我",
    aboutText: "今年 19 歲，目前就讀基隆二信高中。不得不誠實地說，我的脾氣有時候不太好，但如果你走進我的世界，就會發現我對朋友非常友善與義氣。我是一個熱愛用鏡頭記錄生活、在影像剪輯中尋找樂趣，同時也曾在遊戲世界裡瘋狂逐夢的熱血青年。",
    profileNameLabel: "名字 / 綽號",
    profileLocationLabel: "所在地",
    profileLocation: "台灣基隆 / 讀二信高中",
    blogEyebrow: "Blog Notes",
    blogTitle: "最新生活札記",
    blogOneMeta: "攝影日常",
    blogOneTitle: "雨港街頭的光",
    blogOneDesc: "在基隆的雨天裡找光、找故事，也找下一張值得留下的照片。",
    blogTwoMeta: "剪輯練習",
    blogTwoTitle: "把生活剪成節奏",
    blogTwoDesc: "使用 CapCut 慢慢練習，讓畫面、音樂和情緒對上拍。",
    blogThreeMeta: "2026 轉折",
    blogThreeTitle: "停止遊戲後的新目標",
    blogThreeDesc: "把原本放在遊戲裡的熱血，轉成攝影、朋友和未來生活的推進力。",
    skillsEyebrow: "Skills & Gear",
    skillsTitle: "我的專長與攝影裝備",
    photographyTitle: "攝影拍照",
    photographyDesc: "熟練度高。喜歡捕捉街景、人像與生活瞬間。",
    editingTitle: "影片剪輯",
    editingDesc: "初級階段。目前正在鑽研剪輯節奏與特效。",
    gearTitle: "攝影裝備欄",
    gearDesc: "主要使用手機/相機進行日常拍攝，搭配 CapCut 進行初級剪輯。",
    lifeEyebrow: "Campus & Life",
    lifeTitle: "基隆與二信高中生活",
    lifeText: "在雨港基隆的二信高中就讀，這裡是我磨練技術、廣交朋友的地方。雖然課業生活充實，但我總能找到時間拿起相機，記錄這座城市的雨景與校園的點滴。",
    gamingEyebrow: "Gaming Chronicle",
    gamingTitle: "遊戲時光機（2022 - 2025 賽季）",
    gamingNotice: "2022-2025 輝煌遊戲生涯（2026 年因生活重心調整，目前已停止遊戲生活）",
    gameSpeedTitle: "極速領域",
    gameSpeedDesc: "追求極速甩尾的快感，享受在賽道上零點幾秒的博弈。",
    gameRobloxDesc: "自由度極高的世界，在這裡和朋友們留下了超多搞笑和冒險的回憶。",
    gameWeplayDesc: "不僅僅是玩遊戲，更是和朋友用語音打屁、聯絡感情的最佳基地。",
    gamePcTitle: "PC 電腦遊戲特輯",
    gamePcDesc: "從方塊建築、瘋狂飆車到像素射擊，陪伴了我無數個熱血的夜晚。",
    goalsEyebrow: "2026 Future Goals",
    goalsTitle: "2026 全新篇章與未來目標",
    goalOneTitle: "精進攝影與剪輯技術",
    goalOneDesc: "嘗試接案或製作個人短片。",
    goalTwoTitle: "順利從二信高中畢業",
    goalTwoDesc: "迎向 19 歲的成年挑戰。",
    goalThreeTitle: "擴展人際關係",
    goalThreeDesc: "對身邊友善的朋友們更好。",
    contactEyebrow: "Contact",
    contactTitle: "聯絡我"
  },
  en: {
    brandText: "Willie Zhang's Blog",
    navAbout: "About",
    navSkills: "Skills",
    navGallery: "Photography",
    navGaming: "Gaming",
    navGoals: "Future Goals",
    navContact: "Contact",
    heroEyebrow: "Photography Life / Gaming Chronicle / 2026 New Chapter",
    heroTagline: "A student from Erxin High School in Keelung, collecting rainy-city moments through the lens and writing passion into my own story.",
    heroQuote: "Capture the moment with the lens, live the moment with passion.",
    heroCta: "Explore My World",
    heroIg: "Follow Instagram",
    heroStatusTitle: "Current Status",
    heroStatus: "Focused on photography, editing, and personal growth in 2026",
    aboutEyebrow: "About Willie",
    aboutTitle: "About Me",
    aboutText: "I'm 19 years old and currently studying at Erxin High School in Keelung. To be honest, I might have a temper sometimes, but once you get to know me, you'll find I'm extremely loyal and friendly to my friends. I'm a passionate guy who loves capturing life through the lens, finding joy in video editing, and chasing dreams in the gaming world.",
    profileNameLabel: "Name / Nickname",
    profileLocationLabel: "Location",
    profileLocation: "Keelung, Taiwan / Erxin High School",
    blogEyebrow: "Blog Notes",
    blogTitle: "Latest Life Notes",
    blogOneMeta: "Photography Daily",
    blogOneTitle: "Light on the Rainy Streets",
    blogOneDesc: "Finding light, stories, and the next photo worth keeping in Keelung's rainy days.",
    blogTwoMeta: "Editing Practice",
    blogTwoTitle: "Cutting Life Into Rhythm",
    blogTwoDesc: "Using CapCut to practice how images, music, and emotion can land on the same beat.",
    blogThreeMeta: "2026 Reset",
    blogThreeTitle: "New Goals After Gaming",
    blogThreeDesc: "Turning the passion I once put into games into photography, friendships, and the future.",
    skillsEyebrow: "Skills & Gear",
    skillsTitle: "Skills & Photography Gear",
    photographyTitle: "Photography",
    photographyDesc: "Highly skilled. I love capturing streets, portraits, and everyday moments.",
    editingTitle: "Video Editing",
    editingDesc: "Beginner level. Currently learning editing rhythm and visual effects.",
    gearTitle: "My Gear",
    gearDesc: "Primarily using smartphone/camera for daily shooting, paired with CapCut for basic editing.",
    lifeEyebrow: "Campus & Life",
    lifeTitle: "Keelung & Erxin High School Life",
    lifeText: "Studying at Erxin High School in the rainy city of Keelung. This is where I hone my skills and make lifelong friends. Despite the busy school life, I always find time to grab my camera and capture the city's rainy scenery and campus moments.",
    gamingEyebrow: "Gaming Chronicle",
    gamingTitle: "Gaming Time Machine (2022 - 2025 Seasons)",
    gamingNotice: "2022-2025 glorious gaming career (gaming life has currently stopped in 2026 due to a shift in life priorities)",
    gameSpeedTitle: "Speed Drifters",
    gameSpeedDesc: "Chasing the thrill of drifting and competing for split-second victories on the track.",
    gameRobloxDesc: "A world of ultimate freedom where I made countless hilarious and adventurous memories with friends.",
    gameWeplayDesc: "More than just party games; it was the ultimate hangout spot to chill and chat with friends.",
    gamePcTitle: "PC Gaming Special",
    gamePcDesc: "From sandbox building and intense racing to pixel shooting, these games fueled countless passionate nights.",
    goalsEyebrow: "2026 Future Goals",
    goalsTitle: "2026 New Chapter & Future Goals",
    goalOneTitle: "Master photography and video editing",
    goalOneDesc: "Try freelance work or create personal short films.",
    goalTwoTitle: "Graduate smoothly from Erxin High School",
    goalTwoDesc: "Embrace the challenges of turning 19.",
    goalThreeTitle: "Expand my social circle",
    goalThreeDesc: "Be even better to the friendly people around me.",
    contactEyebrow: "Contact",
    contactTitle: "Contact Me"
  }
};

const langButton = document.querySelector(".lang-toggle");
const menuToggle = document.querySelector(".menu-toggle");
const navPanel = document.querySelector(".nav-panel");

function applyLanguage(lang) {
  const dictionary = translations[lang] || translations.zh;
  document.documentElement.lang = lang === "zh" ? "zh-Hant" : "en";

  document.querySelectorAll("[data-i18n]").forEach((element) => {
    const key = element.dataset.i18n;
    if (dictionary[key]) {
      element.textContent = dictionary[key];
    }
  });

  langButton.textContent = lang === "zh" ? "中文 / EN" : "EN / 中文";
  localStorage.setItem("willie-site-lang", lang);
}

langButton.addEventListener("click", () => {
  const current = localStorage.getItem("willie-site-lang") || "zh";
  applyLanguage(current === "zh" ? "en" : "zh");
});

menuToggle.addEventListener("click", () => {
  const isOpen = navPanel.classList.toggle("is-open");
  menuToggle.setAttribute("aria-expanded", String(isOpen));
});

document.querySelectorAll(".nav-panel a").forEach((link) => {
  link.addEventListener("click", () => {
    navPanel.classList.remove("is-open");
    menuToggle.setAttribute("aria-expanded", "false");
  });
});

applyLanguage(localStorage.getItem("willie-site-lang") || "zh");
